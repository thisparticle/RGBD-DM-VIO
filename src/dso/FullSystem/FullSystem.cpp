...
void FullSystem::addActiveFrame(ImageAndExposure* image,ImageAndExposure* depthImage, int id, dmvio::IMUData* imuData, dmvio::GTData* gtData)
{
	...
	if(!initialized)
	{
		if(coarseInitializer->frameID<0)	// first frame set. fh is kept by coarseInitializer.
		{
			...
			std::cout << "coarseInitializer finished!" << std::endl;
			initialized = true;
        }
		return;
		// use initializer!
	...
	else	// do front-end operation.
	{
		if(allFrameHistory.size() == 2){
			...
			std::pair<Vec4, bool> pair = trackNewCoarse(fh);
			lock.unlock();
			deliverTrackedFrame(fh, true);
			return;
		}
